...
class BaseSerializer(serpy.DictSerializer):
    ...
    def __protect_data(self, protected_fields, data):
        try:
    ...
    def valid_read_data(self):
        if self.__is_valid():
            valid_data =  self.__protect_data( protected_fields = self.Meta.read_protected_fields, data = self.data )
            return valid_data

    @property
    def valid_write_data(self):
       if self.__is_valid():
            valid_data =  self.__protect_data( protected_fields = self.Meta.write_protected_fields, data = self.data )
            return valid_data
